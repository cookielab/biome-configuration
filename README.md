...
The individual packages can then just extend from it with the `//` shorthand syntax. You can optionally include the `"root": false` flag as well. However, Biome sets it implicitly to `false` when it encounters `//` in the `extends` field.

```json
{
  "root": false,
  "extends": ["//", "@cookielab.io/biome-config/react"]
}
```
